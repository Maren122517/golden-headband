...
if (botonesBuscadorPorNombre) {
    for (let i = 0; i < botonesBuscadorPorNombre.length; i++) {

        if (botonesBuscadorPorNombre[i] && botonesBuscadorPorNombre[i].onchange) {
            botonesBuscadorPorNombre[i].onchange = () => {

                if (botonesBuscadorPorNombre[i] && botonesBuscadorPorNombre[i].checked === true) {

                    let textoBuscador = eliminarAcentos((nombreBuscadorPorNombre[i] && nombreBuscadorPorNombre[i].textContent) ? nombreBuscadorPorNombre[i].textContent.toLowerCase() : "");

                    for (let o = 0; o < botonesBuscadorPorNombre.length; o++) {

                        if (botonesBuscadorPorNombre[i] !== botonesBuscadorPorNombre[o]) {
                            botonesBuscadorPorNombre[o].checked = false;  // Cambié null a false para mayor claridad.
                        }
                    }
                    ...
                    for (let o = 0; o < cardsTexto.length; o++) {
                        ...
                        if (cardsCreadas[o]) {
                            ...
                            let textoCard = eliminarAcentos((cardsTexto[o] && cardsTexto[o].textContent) ? cardsTexto[o].textContent.toLowerCase() : "");
